...
def subseqs(s):
    ...
    if not s:
        ...
        without_first_elem = subseqs(s[1:]) # Give me all the subsequences that don't have the first element
        with_first_elem = insert_into_all(s[0], subseqs(s[1:])) # Give me all the subsequences that atleast contain the first element
        return without_first_elem + with_first_elem # Bring them together!
...
def non_decrease_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            return [[]]
        elif s[0] < prev: # Indicating that our previous element is greater so we need to disregard this subseq
            return subseq_helper(s[1:], prev)
        else:
            a = subseq_helper(s[1:], s[0]) # With first elem (need to add it back at the end!)
            b = subseq_helper(s[1:], prev) # Without first elem
            return insert_into_all(s[0], a) + b
    return subseq_helper(s, -5) # Pass in the list (s) and -1 to kickstart prev

    # Short solution (I made, John Denero said he might put it as another solution, lol))
    return list(filter(lambda lst: lst == sorted(lst), subseqs(s)))
    # Another short solution
    return [subseq for subseq in subseqs(s) if subseq == sorted(subseq)]
...
def insert(link, value, index):
    ...
    """
    if link.rest is Link.empty and index:
