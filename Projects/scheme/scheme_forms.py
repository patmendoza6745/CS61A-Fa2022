...
def do_if_form(expressions, env):
    ...
    if is_scheme_true(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.rest.first, env, True) # TAIL CONTEXT: RETURNING THE CONSEQUENT
    elif len(expressions) == 3:
        return scheme_eval(expressions.rest.rest.first, env, True) # TAIL CONTEXT: RETURNING THE ALTERNATIVE
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        return scheme_eval(expressions.first, env, True) # TAIL CONTEXT: RETURNING THE LAST VALUE
