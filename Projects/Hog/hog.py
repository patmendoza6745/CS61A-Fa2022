...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        num_rolls -= 1
        total += number
    return 1 if is_dice_one else total
    # END PROBLEM 1
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 3
    return (roll_dice(num_rolls, dice)) if num_rolls > 0 else tail_points(opponent_score)
    # END PROBLEM 3
...
def perfect_square(score):
    return int(score**0.5) == score**0.5
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    num_rolls = 0
    for n in range(1,11):
        result = make_averaged(roll_dice,total_samples)(n, dice)
        if result > max_avg:
            num_rolls = n
            max_avg = result
    return num_rolls
...
def final_strategy(score, opponent_score):
    ...
    This strategy returns 0 dice if the abs. value of the difference between the tens and ones of the opponent's score
    is greater than 5. Otherwise, return 6.
    """ 
    # BEGIN PROBLEM 12
    return 0 if abs(opponent_score//10%10 - opponent_score%10) >= 5 else 6
    # END PROBLEM 12
