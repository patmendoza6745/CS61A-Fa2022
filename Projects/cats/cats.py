...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    filtered = [paragraph for paragraph in paragraphs if select(paragraph)]
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    return lambda text: bool([True for word in remove_punctuation(text.lower()).split() if word in topic])
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    correct = 0
    iterate = min(len(typed_words), len(source_words))

    if not typed_words and not source_words:
        return 100.0
    elif typed_words and not source_words or not typed_words and source_words:
        return 0.0
    else:
...
def feline_fixes(typed, source, limit):
    ...
    if not typed and not source:
        return 0
    elif typed and not source or not typed and source:
        return abs(len(typed)-len(source))
    elif limit < 0:
        return 1
    elif typed[0] != source[0]:
        return 1 + feline_fixes(typed[1:], source[1:], limit-1) if limit != 0 else 1
...
def final_diff(typed, source, limit):
    ...
    if typed == source or not typed and not source:
        return 0
    elif typed[0] == source[1] and source[0] == typed[1]:
        return 1
    elif typed[0] == source[1] and typed[0] == source[0]:
        return 1
    else:
        if limit == 0:
            return 1
...
def report_progress(typed, prompt, user_id, upload):
    ...
    while prompt and typed and typed[0] == prompt[0]:
        correct += 1
        typed = typed[1:]
        prompt = prompt[1:]
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    return match(words, [[lst[i+1]-lst[i] for i in range(len(lst)-1)] for lst in times_per_player])
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for i in player_indices:
        ...
        for j in word_indices:
            if i == min([k for k in player_indices], key=lambda k: time(match, k, j)):
                lst.append(get_word(match, j))
        fastest_words.append(lst)
